					  ...
					  )
{
	...
	do 
	{
		if (_BitScanForward(&Index, Mask))
		{
			...
			{
				if (0 <= (status = NtOpenProcess(&hProcess, 
					PROCESS_VM_OPERATION|PROCESS_CREATE_THREAD|PROCESS_QUERY_INFORMATION|PROCESS_SET_INFORMATION, 
					const_cast<POBJECT_ATTRIBUTES>(&zoa), &cid)))
				{
				...
				{
					if (0 <= NtOpenProcess(&hProcess, PROCESS_QUERY_LIMITED_INFORMATION, const_cast<POBJECT_ATTRIBUTES>(&zoa), &cid))
					{
						set_cmdline(pspi, GetCommandLine(hProcess));
						NtClose(hProcess);
					}
